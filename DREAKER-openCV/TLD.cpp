...
using namespace cv;
using namespace std;
char target[9] = { 0xff,0xfe,0,0,0,0,0,0,0xfd };
		...
		if (detected)                                                //   if Detected
		{
			...
			else
			{
				...
				if (close_detections>0)
				{
					...
					if ((tbb.x + tbb.width / 2 - 160) < 0) target[2] = 1;
					if ((tbb.x + tbb.width / 2 - 160) >=0) target[2] = 0;					
					target[3] = abs(tbb.x + tbb.width/2 - 160) / 256;
					target[4] = abs(tbb.x + tbb.width/2 - 160) % 256;
					if ((tbb.y + tbb.height / 2 - 120) < 0) target[5] = 1;
					if ((tbb.y + tbb.height / 2 - 120) >=0) target[5] = 0;
					target[6] = abs(tbb.y + tbb.height /2 - 120) / 256;
					target[7] = abs(tbb.y + tbb.height /2 - 120) % 256;
					//target[2] = -58 / 128;
					//target[3] = -58 % 128;
					//target[4] = 58 / 128;
					//target[5] = 58 % 128;
...
void TLD::bbPoints(vector<cv::Point2f>& points, const BoundingBox& bb)
{
	...
	int margin_v = 0;
	//网格均匀撒点
	int stepx = ceil((double)(bb.width - 2 * margin_h) / max_pts);  //ceil返回大于或者等于指定表达式的最小整数
	int stepy = ceil((double)(bb.height - 2 * margin_v) / max_pts);
	//int stepx = ceil((double)(bb.width - 2 * margin_h) / 8);  //ceil返回大于或者等于指定表达式的最小整数
	//int stepy = ceil((double)(bb.height - 2 * margin_v) / 6);
	...
	const BoundingBox& bb1, BoundingBox& bb2)
{
	...
	////////////////////////////////////////////
	//此处进行修改过，round->floor(x+0.5)
	////////////////////////////////////////////
	bb2.x = floor(bb1.x + dx - s1 + 0.5);
	bb2.y = floor(bb1.y + dy - s2 + 0.5);
	bb2.width = floor(bb1.width*s + 0.5);
	bb2.height = floor(bb1.height*s + 0.5);
...
void TLD::buildGrid(const cv::Mat& img, const cv::Rect& box)
{
	...
	for (int s = 0; s < 21; s++)
	{
		width = floor(box.width*SCALES[s] + 0.5);
		height = floor(box.height*SCALES[s] + 0.5);
		...
		scales.push_back(scale);  //把该尺度的窗口存入scales容器，避免在扫描时计算，加快检测速度
								  ...
								  //此处进行修改过，round->floor(x+0.5)
								  ////////////////////////////////////////////
		for (int y = 1; y<img.rows - height; y += floor(SHIFT*min_bb_side + 0.5))   //按步长移动窗口
		{
			for (int x = 1; x<img.cols - width; x += floor(SHIFT*min_bb_side + 0.5))
			{
				bbox.x = x;
				bbox.y = y;
				bbox.width = width;
